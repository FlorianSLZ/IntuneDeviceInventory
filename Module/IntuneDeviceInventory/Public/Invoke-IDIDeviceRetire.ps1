function Invoke-IDIDeviceRetire {

    <#
    .SYNOPSIS
        Retire Intune devices (single or bulk)
    ...
    param (
        ...
        [parameter(Mandatory = $false, HelpMessage = "Retire without asking for each device")]
        [ValidateNotNullOrEmpty()]
        [switch]$Force
    ...
    if($All){
        if($global:IDIDevices_all){$global:IDIDevices_all | ForEach-Object{ Invoke-IDIDeviceRetire -IDIDevice $_}}
        else{Get-noneIDIDevice -All | ForEach-Object{ Invoke-IDIDeviceRetire -IDIDevice $_}}
        
    }elseif($Group){
        Get-noneIDIDevice -Group "$Group" | ForEach-Object{ Invoke-IDIDeviceRetire -IDIDevice $_}
        
    }elseif($User){
        Get-noneIDIDevice -User "$User" | ForEach-Object{ Invoke-IDIDeviceRetire -IDIDevice $_}
        
    }elseif($deviceName){
        Get-noneIDIDevice -deviceName "$deviceName" | ForEach-Object{ Invoke-IDIDeviceRetire -IDIDevice $_}
        
    }elseif($id){
        Get-noneIDIDevice -id "$id" | ForEach-Object{ Invoke-IDIDeviceRetire -IDIDevice $_}
        
    }elseif($azureADDeviceId){
        Get-noneIDIDevice -azureADDeviceId "$azureADDeviceId" | ForEach-Object{ Invoke-IDIDeviceRetire -IDIDevice $_}
        ...
        if($global:IDIDevices_all){
            ...
            $Devices2Sync | ForEach-Object{ Invoke-IDIDeviceRetire -IDIDevice $_}
        }
        else{Get-noneIDIDevice -Grid | ForEach-Object{ Invoke-IDIDeviceRetire -IDIDevice $_}}
        
    }elseif($IDIDevice){

        if(!$Force){
            $RunMajor = Read-Host "Are you sure you want to retire Device $($IDIDevice.id) (Y/N)"
